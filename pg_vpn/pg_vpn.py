...
class VPN(Q):
    ...
    def define_loss(self):
        ...
        self.entropy = -tf.reduce_sum(self.probs * tf.log(self.probs), axis=1, name="entropy")
        self.cross_entropy_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logprobs, labels=self.taken_actions)
        self.pg_loss = tf.reduce_mean(self.cross_entropy_loss + self.entropy_reg * self.entropy)
