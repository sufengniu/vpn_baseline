...
def transform_conv_state(x, a, n_actions, top_k, filter_size=(3, 3), pad="SAME"):
    ...
    b_gate = tf.get_variable("gate-b", [1, 1, 1, num_filters],
                    initializer=tf.constant_initializer(0.0))
    if a is not None:
        if len(a.get_shape()) == 2: # for learning
            idx = tf.to_int32(tf.squeeze(tf.argmax(a, axis=-1)))
            conv = tf.nn.conv2d(x, w[:, :, :, idx, :], stride_shape, pad) + b[:, :, :, idx, :]
            conv = act_fn(conv)
        elif len(a.get_shape()) == 3: # top_k policy candidates
            idx_candidates = tf.to_int32(tf.argmax(a, axis=-1))
            range_idx = tf.expand_dims(tf.tile(
                        tf.expand_dims(tf.range(tf.shape(idx_candidates)[0]), -1), [1,top_k]), -1)
            idx = tf.concat([range_idx, tf.expand_dims(idx_candidates, -1)], -1)
            ...
            conv = tf.transpose(tf.reshape(conv, 
                        [-1, xdim[0], xdim[1], n_actions, num_filters]), 
                        [0, 3, 1, 2, 4])
            conv = pack(tf.gather_nd(conv, idx), xdim)
    else:
        w = tf.reshape(w, [filter_size[0], filter_size[1], num_filters, n_actions * num_filters])
        b = tf.reshape(b, [1, 1, 1, n_actions * num_filters])
        conv = act_fn(tf.nn.conv2d(x, w, stride_shape, pad) + b)
    ...
    if a is not None:
        if len(a.get_shape()) == 2: # for learning
            ...
            conv = tf.transpose(tf.reshape(conv, [-1, top_k] + xdim), [1, 0, 2, 3, 4])
            gate = tf.transpose(tf.reshape(gate, [-1, top_k] + xdim), [1, 0, 2, 3, 4])
            conv = conv * gate + x
...
def transform_conv_pred(x, a, n_actions, top_k, filter_size=(3, 3), pad="SAME"):
...
def to_policy(x, n_actions, dim=256, initializer=None, bias_init=0):
    x = act_fn(linear(flatten(x), dim, "pi1", 
            initializer=tf.contrib.layers.xavier_initializer(), bias_init=bias_init))
    return linear(x, n_actions, "pi", initializer=initializer, bias_init=bias_init)

def rollout_step(x, a, n_actions, op_trans_state, op_trans_pred,
                op_value, op_steps, op_reward, op_policy, top_k, gamma=0.98):
    ...
    if a is not None:
        ...
        a_prob = tf.nn.softmax(op_policy(x))
        _, a_idx = tf.nn.top_k(a_prob, k=top_k)
        a = tf.one_hot(a_idx, n_actions)

        state = op_trans_state(x, a)
        p = op_trans_pred(x, a)

        v_next = pack(op_value(state), [top_k])
        r = pack(op_reward(p), [top_k])
        t = pack(op_steps(p), [top_k])
   
    t = tf.nn.relu(t) + 1
    g = tf.pow(tf.constant(gamma), t)
    return r, g, t, v_next, state, a
...
def predict_over_time(x, a, n_actions, op_rollout, prediction_step=5):
    ...
    def _time_step(time, r_ta, g_ta, t_ta, v_ta, q_ta, s_ta, state):
        ...
        r, gamma, t, v_next, state, _ = op_rollout(state, a_t)
        q = r + gamma * v_next
...
class Model(object):
    def __init__(self, ob_space, n_actions, type, 
        ...
        self.branch = [min(n_actions, k, top_k) for k in branch]
        self.top_k = top_k
        ...
        if self.type == 'policy':
        ...
        elif self.type == 'pg_vpn':
            self.op_policy = tf.make_template('pi', to_policy, dim=self.dim,
                    n_actions=n_actions, initializer=normalized_columns_initializer(0.01))
            ...
            self.op_trans_state = tf.make_template('trans_state', transform_conv_state, 
                    n_actions=n_actions, top_k=top_k)
            ...
            self.op_rollout = tf.make_template('rollout', rollout_step, 
                    ...
                    op_policy=self.op_policy,
                    top_k=self.top_k,
                    gamma=gamma)

            self.vf = self.op_value(self.s)

            # Unconditional rollout
            self.r, self.gamma, self.steps, self.v_next, self.state, self.pi = self.op_rollout(self.s, None)
            ...
            # model-free
            self.logits = self.op_policy(self.s)
            self.sample = categorical_sample(self.logits, n_actions)[0, :]
            ...
            for i in range(depth):
                r, gamma, _, v, s, pi_a = self.op_rollout(s, None)
                pi_a = tf.argmax(pi_a, -1)
                ...
                b = min(self.branch[i], self.top_k)
                q_list.append(r_list[i] + g_list[i] * v_list[i])
                q_list[i] = tf.reshape(q_list[i], [-1, self.top_k])
                _, idx_b = tf.nn.top_k(q_list[i], k=b)

                range_idx = tf.expand_dims(tf.tile(tf.expand_dims(tf.range(tf.shape(idx_b)[0]), -1), [1,b]), -1)
                idx_idx = tf.concat([range_idx, tf.expand_dims(idx_b, -1)], -1)
                idx = tf.to_int32(tf.gather_nd(a_list[i], idx_idx))
                idx_list.append(idx)
                ...
                s = tf.reshape(tf.gather_nd(
                        tf.reshape(s, [-1, self.top_k] + self.sdim), l), [-1] + self.sdim)
                r_list[i] = tf.reshape(tf.gather_nd(
                        tf.reshape(r_list[i], [-1, self.top_k]), l), [-1])
                g_list[i] = tf.reshape(tf.gather_nd(
                        tf.reshape(g_list[i], [-1, self.top_k]), l), [-1])
                v_list[i] = tf.reshape(tf.gather_nd(
                        tf.reshape(v_list[i], [-1, self.top_k]), l), [-1])
                a_list[i] = tf.reshape(tf.reshape(a_list[i], [-1, self.top_k]), [-1])
            ...
            a_plan = tf.squeeze(a_list[0])
            self.q_deep = tf.squeeze(q_plan[0])
            # policy plan
            self.pi_plan = tf.sparse_to_dense(a_plan, [self.n_actions], self.q_deep, 
    ...
    def value(self, ob, state_in=[], meta=None):
        ...
        if self.type == 'policy':
            ...
            q, q_pi = sess.run([self.q_plan, self.pi_plan], feed_dict)
            return q.max()
